...
public class StationController {
    ...
    @PostMapping("/add")
    public ResponseEntity addStation(@Valid @RequestBody Station station , Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateStation(@PathVariable Integer id,@Valid @RequestBody Station station , Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
