...
public class TrainController {
    ...
    @PostMapping("/add")
    public ResponseEntity addTrain(@Valid @RequestBody Train train , Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateTrain(@PathVariable Integer id,@Valid @RequestBody Train train,Errors errors ) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
