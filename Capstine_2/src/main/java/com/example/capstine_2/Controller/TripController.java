...
public class TripController {
    ...
    @PostMapping("/add")
    public ResponseEntity addTrip(@Valid @RequestBody Trip trip , Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateTrip(@PathVariable int id,@Valid @RequestBody Trip trip, Errors errors ) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
