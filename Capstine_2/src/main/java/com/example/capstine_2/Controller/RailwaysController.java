...
public class RailwaysController {
    ...
    @PostMapping("/add")
    public ResponseEntity addRailways(@Valid @RequestBody Railways railways , Errors errors) {
        if (errors.hasErrors()) {
    ...
    public ResponseEntity updateRailways(@PathVariable Integer id,@Valid @RequestBody Railways railways , Errors errors) {
        if (errors.hasErrors()) {
            return ResponseEntity.status(400).body(errors.getFieldError().getDefaultMessage());
        }
